...
function TripsList() {
  ...
  const ongoing = useMemo(
    () => trips.filter((tr) => tr.start_date <= today && tr.end_date >= today).sort((a, b) => b.start_date.localeCompare(a.start_date)),
    [trips, today],
  );
  const planned = useMemo(
    () => trips.filter((tr) => tr.start_date > today).sort((a, b) => a.start_date.localeCompare(b.start_date)),
    [trips, today],
  );
  const past = useMemo(
    () => trips.filter((tr) => tr.end_date < today).sort((a, b) => b.start_date.localeCompare(a.start_date)),
    [trips, today],
  );
    ...
    <main className="mx-auto max-w-6xl px-4 py-6 sm:py-8">
      ...
      {q.isLoading ? (
        ...
        <div className="mt-8 space-y-8">
          ...
          {past.length > 0 && <Section title={t("past")} trips={past} accent="muted" withYearSelector />}
        </div>
      )}
    </main>
  );
...
function TripCard({ trip, carousel = false }: { trip: Trip; carousel?: boolean }) {
  ...
  const tripType = ((trip as unknown as { trip_type?: string }).trip_type ?? "vacation") as "vacation" | "business" | "daytrip";
